...
- Test 4 (sustained): Max 12ms vs P99 5ms ✓ (no spike)
...
**Investigation Tasks:**
- [ ] Add instrumentation to measure time-to-accept for incoming connections
- [ ] Profile memory allocation patterns during burst scenarios
- [ ] Examine TCP accept queue size and backlog configuration
...
**Acceptance Criteria:**
- [ ] Zero timeouts at 50 workers over 5000 messages
- [ ] Documented backlog configuration in deployment guide
- [ ] Optional: Metrics for connection queue depth
- [ ] Optional: Graceful overload handling (421 responses)
...
**Acceptance Criteria:**
- [ ] Emit structured logs for all recommended metrics
- [ ] Document log format and metrics definitions
- [ ] Provide example monitoring configuration (Prometheus/Grafana)
- [ ] Add metrics extraction documentation

**Release Impact:**
Not a blocker, but highly recommended for any production deployment. Can be added in v0.2.1 without breaking changes.
...
**Description:**
The report notes a 4% throughput regression from v0.1.0 (685 msg/sec → 659 msg/sec) and attributes it to "TLS overhead." This assumption should be validated with comparative testing.

**Investigation Tasks:**
- [ ] Run stress tests with TLS disabled (plaintext SMTP)
- [ ] Compare throughput: v0.1.0, v0.2.0-plaintext, v0.2.0-TLS
- [ ] Profile TLS handshake and encryption overhead
- [ ] Validate that regression is solely due to TLS, not other changes
- [ ] Document actual TLS performance cost
...
**Acceptance Criteria:**
- [ ] Documented performance comparison (plaintext vs TLS)
- [ ] Root cause analysis of throughput regression
- [ ] Optimization opportunities identified (if any)
- [ ] Updated performance benchmarks in documentation

**Release Impact:**
Not a blocker. The 4% difference is within noise margins and v0.2.0 performance is still excellent. However, validating the assumption ensures no hidden regressions exist.
...
- ✅ Complete H2 (Production Deployment Guide) - documentation only
- ✅ Update README with performance characteristics
- ✅ Document known limitations (50+ worker burst scenarios)

### 🔄 v0.2.1 Stability Release (Recommended)
